...
export function OrderTabs({ tabs, pathname }: OrderTabsProps) {
  ...
  return (
    <div className="mb-4">
      {isMobile ? (
        // Mobile: Grid layout with exactly 2 columns
          ...
          {tabs.map((tab) => {
            ...
            return (
              <motion.button
                ...
                className={`
                  relative px-3 py-2 cursor-pointer rounded-lg font-medium text-xs transition-all duration-200 ease-in-out
                ...
                whileTap={{ scale: 0.98 }}
                title={tab.label} // Show full label on hover/touch
              >
                {tab.label}
                {isActive && (
                  <motion.div
