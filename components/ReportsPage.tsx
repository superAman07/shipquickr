...
import { Copy, Trash2, Search, Package, Home, ChevronRight, Download, MoreHorizontal, DownloadCloud } from "lucide-react";
...
type TabKey = "all" | "in_transit" | "out_for_delivery" | "unshipped" | "delivered" | "undelivered" | "rto_intransit" | "rto_delivered" | "lost_shipment";
...
function gs(s: string) {
  return SS[s.toLowerCase()] || { bg: 'bg-gray-500/10', text: 'text-gray-600 dark:text-gray-400', dot: 'bg-gray-500' };
}
...
const ReportsPage: React.FC = () => {
  ...
  function downloadCSV(ordersToDownload: Order[]) {
    ...
    const csvContent = [headers.join(','), ...rows.map(row => row.map(field => `"${(field ?? "").toString().replace(/"/g, '""')}"`).join(','))].join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", `${currentTabObj.label.toLowerCase().replace(/ /g, '-')}-report.csv`);
      ...
      <div className="px-4 pb-0 pt-4 sm:px-6 sm:pt-6">
        ...
        <div className="mb-4 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div className="min-w-0 flex-1">
            <h1 className="text-2xl font-extrabold tracking-tight text-gray-800 dark:text-white sm:text-3xl">Reports</h1>
            <div className="mt-1.5 flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
              <Link href="/user/dashboard" className="flex items-center transition-colors hover:text-gray-700 dark:hover:text-gray-200">
                <Home className="mr-1 h-3 w-3" /> Dashboard
              </Link>
              <ChevronRight className="mx-0.5 h-3 w-3" />
              <span>Reports</span>
              <ChevronRight className="mx-0.5 h-3 w-3" />
              <span className="font-medium text-gray-700 dark:text-gray-300 capitalize">{currentTabObj.label.toLowerCase()}</span>
            </div>
          </div>
        ...
        <div className="shadow-xs overflow-hidden rounded-xl rounded-tl-none border border-gray-200 bg-white dark:border-gray-800 dark:bg-[#111827]">
            ...
            <div className="max-h-[calc(100vh-300px)] overflow-y-auto">
              <div className="space-y-3 p-4">
                {paginatedOrders.map((order) => (
                  <div key={order.id} className="rounded-xl border border-gray-200 bg-gray-50 p-4 shadow-sm transition-transform active:scale-[0.99] dark:border-gray-700 dark:bg-gray-800" onClick={(e) => handleRowClick(order, e)}>
                    ...
                    <div className="mb-3 grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <div className="mb-0.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Customer</div>
                        <div className="font-medium text-gray-800 dark:text-gray-100">{order.customerName}</div>
                        <div className="text-[11px] text-gray-500 dark:text-gray-400">{order.mobile}</div>
                      </div>
                      <div>
                        <div className="mb-0.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Payment</div>
                        <div className="font-medium text-gray-800 dark:text-gray-100">{order.paymentMode || "Prepaid"}</div>
                      </div>
                    </div>

                    <div className="mb-3">
                      <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Products</div>
                      <div className="rounded-lg border border-gray-100 bg-white p-2 dark:border-gray-600 dark:bg-gray-700/50">
                        {order.items.length > 0 ? (
                          <div className="space-y-1 text-xs">
                            {order.items.map((item, idx) => (
                              <div key={idx} className={idx > 0 ? 'border-t border-gray-200 pt-1 dark:border-gray-600' : ''}>
                                <span className="font-medium text-gray-800 dark:text-gray-100">{item.productName}</span> ({item.quantity}x)
                                {item.hsn && <span className="block text-[10px] text-gray-400">HSN: {item.hsn}</span>}
                              </div>
                            ))}
                          </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2 border-t border-gray-200 pt-3 dark:border-gray-600">
                      <button onClick={(e) => { e.stopPropagation(); handleCloneOrder(order.id); }} className="flex flex-1 justify-center cursor-pointer items-center gap-1 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 transition hover:bg-blue-100 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800">
              ...
              <table className="min-w-full text-left border-separate border-spacing-0 whitespace-nowrap">
                <thead className="sticky top-0 z-20 shadow-sm">
                  <tr className="text-white">
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-200">Date/ID</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-200 min-w-[200px]">Product Details</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-200">Order Value</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-200">Customer Details</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-gray-200">Billable Weight</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-gray-200">Ageing</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-gray-200">Attempts</th>
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-200 max-w-[150px]">Shipping Details</th>
                    ...
                    <th className="bg-[#0a0c37] border-b border-gray-800 px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-gray-200">Action</th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-[#111827]">
                  {paginatedOrders.map((order) => {
                    const totalValue = calculateTotalOrderValue(order.items);
                    return (
                      <tr key={order.id} onClick={(e) => handleRowClick(order, e)} className="group hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-colors cursor-pointer relative align-top">
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800">
                          <span className="font-bold text-[#0a0c37] dark:text-indigo-400 text-[13px] block">{order.orderId}</span>
                          <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{new Date(order.orderDate).toLocaleDateString()}</span>
                        </td>
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800 whitespace-normal min-w-[200px]">
                          {order.items && order.items.length > 0 ? (
                            <div className="space-y-1">
                              {order.items.map((item, idx) => (
                                <div key={idx} className={`text-[12px] text-gray-800 dark:text-gray-200 ${idx > 0 ? "pt-1 border-t border-gray-100 dark:border-gray-800" : ""}`}>
                                  <span className="font-semibold uppercase tracking-tight">{item.productName}</span> <span className="text-gray-500">x{item.quantity}</span>
                        ...
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800">
                           <div className="text-[13px] font-bold text-gray-800 dark:text-gray-200">{"\u20B9"}{totalValue.toFixed(0)}</div>
                           <div className="text-[10px] font-semibold text-gray-500 mt-0.5 uppercase tracking-wide">{order.paymentMode || "PREPAID"}</div>
                        </td>
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800">
                          <div className="text-[13px] font-bold uppercase text-gray-900 dark:text-gray-100">{order.customerName}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{order.mobile}</div>
                        </td>
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800 text-center font-medium text-gray-600">{order.billableWeight || "—"}</td>
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800 text-center font-medium text-gray-600">{order.ageing || "0"}</td>
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800 text-center font-medium text-gray-600">{order.attempts || "0"}</td>
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800">
                           <div className="text-[12px] font-bold text-[#0a0c37] dark:text-indigo-400">{order.awbNumber || "—"}</div>
                           <div className="text-[11px] font-medium text-gray-500 mt-0.5 max-w-[150px] truncate" title={order.courierName || order.shippingDetails || ""}>{order.courierName || order.shippingDetails || "—"}</div>
                        </td>
                        ...
                        <td className="px-4 py-4 border-b border-gray-200 dark:border-gray-800 text-center">
                           <div className="inline-flex justify-center -space-x-px rounded-xl shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-700 mx-auto" onClick={(e) => e.stopPropagation()}>
                             <button onClick={() => handleCloneOrder(order.id)} className="relative inline-flex items-center px-3 py-2 text-sm font-semibold text-gray-900 dark:text-gray-100 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 dark:ring-gray-700 focus:z-10 rounded-l-xl cursor-pointer" title="Clone">
                             ...
                             <DropdownMenu>
                               ...
                               <DropdownMenuContent align="end" className="w-48 p-1 z-[80]">
                                 <DropdownMenuItem onClick={() => toast.info("Delete managed from Orders natively")} className="text-red-600 focus:bg-red-50 focus:text-red-700 cursor-pointer rounded-lg">
                  ...
                  {filteredOrders.length === 0 && (
                    <tr>
                      <td colSpan={10} className="py-16 text-center border-b border-gray-200 dark:border-gray-800">
                        <Package className="mx-auto mb-3 h-12 w-12 text-gray-300 dark:text-gray-600" />
                        <h3 className="mb-1 text-base font-semibold text-gray-500 dark:text-gray-400">No records found</h3>
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="px-4 sm:px-6 py-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 flex flex-col sm:flex-row items-center justify-between gap-3">
             <div className="text-xs font-medium text-gray-500 dark:text-gray-400">
               Showing <span className="font-bold text-gray-700 dark:text-gray-300">{filteredOrders.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}</span> {"\u2013"} <span className="font-bold text-gray-700 dark:text-gray-300">{Math.min(currentPage * itemsPerPage, filteredOrders.length)}</span> of <span className="font-bold text-gray-700 dark:text-gray-300">{filteredOrders.length}</span> reports
             </div>
             <div className="flex items-center gap-1.5">
               <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 transition-colors cursor-pointer">Previous</button>
               <span className="px-3 py-1.5 rounded-lg text-xs font-bold bg-[#0a0c37] text-white min-w-[50px] text-center">{currentPage}</span>
               <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage >= totalPages} className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 transition-colors cursor-pointer">Next</button>
             </div>
          </div>
        </div>
      </div>
