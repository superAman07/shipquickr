            ...
            <div className="shrink-0 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white px-6 py-5 dark:border-gray-800 dark:from-[#0a0c37]/50 dark:to-[#111827]">
              ...
              <div className="mt-4 flex border-b border-gray-200 dark:border-gray-800">
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-bold uppercase cursor-pointer tracking-wider transition-colors ${
                    activeTab === "details"
                  ...
                  >
                    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 p-6 shadow-xl shadow-indigo-500/20">
                      ...
                      <div className="relative z-10 flex h-full flex-col justify-between">
                        <div>
                          ...
                          <div className="break-all font-mono text-xl font-bold leading-tight tracking-tight text-white">
                    ...
                    <div className="relative rounded-2xl border-2 border-slate-100 bg-white p-2 shadow-sm before:absolute before:inset-0 before:-z-10 before:rounded-2xl before:bg-gradient-to-b before:from-indigo-50 before:to-white dark:border-slate-800 dark:bg-slate-900 before:dark:from-indigo-950/20 before:dark:to-slate-900">
                      {isLoadingTracking ? (
                        ...
                        (() => {
                          let timeline = [...trackingHistory];

                          // Inject standard base milestones logically derived from order context
                          if (
                            !timeline.some(
                              (e) =>
                                e.status.toLowerCase().includes("order") ||
                                e.status.toLowerCase().includes("received")
                            )
                          ) {
                            timeline.push({
                          ...
                          if (
                            ...
                            !timeline.some(
                              (e) =>
                                e.status.toLowerCase().includes("shipped") ||
                                e.status.toLowerCase().includes("manifest")
                            )
                          ) {
                            timeline.push({
                              status: "Shipment Processed",
                              description: `Assigned Tracking ID: ${order.awbNumber}`,
                              timestamp: new Date(
                                new Date(order.orderDate).getTime() + 60000
                              ).toISOString(),
                              location: order.warehouse?.warehouseName || "Warehouse",
                            });
                          }
                          ...
                          const isOut =
                            currentStatus === "out_for_delivery" || isDelivered;

                          if (
                            isOut &&
                            !timeline.some((e) =>
                              e.status.toLowerCase().includes("out for delivery")
                            )
                          ) {
                            // Find the delivery event to base the time off of, otherwise use now or a fallback
                            const delEvent = timeline.find((e) =>
                              e.status.toLowerCase().includes("delivered")
                            );
                            const outTime =
                              delEvent && delEvent.timestamp
                                ? new Date(
                                    new Date(delEvent.timestamp).getTime() -
                                      3600000 * 3
                                  ).toISOString()
                                : new Date(
                            ...
                            timeline.push({
                              ...
                              description:
                                "Courier partner is out to deliver your shipment",
                              timestamp: outTime,
                            });
                          }
