...
export async function GET(req: NextRequest) {
    ...
    const orders = await prisma.order.findMany({
        where: {
            status: { in: ["undelivered", "rto_intransit", "rto_delivered", "lost_shipment", "delivered"] },
    ...
    const ordersWithCourier = orders.map(order => ({
        ...
        ndrReason: order.ndrReason || null,
        ndrAction: order.ndrAction || null,
        ndrActionRemarks: order.ndrActionRemarks || null,
    }));
