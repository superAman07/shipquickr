  ...
  try {
    ...
    try {
      if (updatedUser.email && updatedUser.firstName) {
        sendKycStatusUpdateEmail(updatedUser.email, updatedUser.firstName, status.toLowerCase()).catch(console.error);
      }
    } catch (emailErr) {
      console.error('Failed to trigger KYC status email:', emailErr);
    }
