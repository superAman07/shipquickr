...
function mapAggregatorStatusToStandard(status?: string): string | null {
  if (!status) return null;
  const s = status.toLowerCase();

  if (s.includes("delivered") && !s.includes("rto") && !s.includes("undelivered")) return "delivered";
  if (s.includes("out for delivery")) return "out_for_delivery";
  if (s.includes("in transit") || s.includes("shipped") || s.includes("pickup done")) return "in_transit";
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`Cron: Found ${orders.length} orders to track.`);
            ...
            await prisma.$transaction([
              prisma.order.update({
                ...
                data: {
                  ...
                  shippingDetails: trackingData.description || `Updated to ${normalizedStatus}`
                }
              }),
              prisma.orderTracking.create({
                data: {
                  ...
                  courier: order.courierName || "Aggregator",
                  timestamp: new Date()
                }
              })
            ]);
            return { id: order.id, status: normalizedStatus };
          }
        }
        return { id: order.id, status: "unchanged" };
      })
    );
