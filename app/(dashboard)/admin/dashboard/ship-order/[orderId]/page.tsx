...
export default function AdminShipOrderPage() {
  ...
  useEffect(() => {
    const fetchOrderAndRates = async () => {
      ...
      try {
        ...
        const orderRes = await axios.get(`/api/admin/orders/${orderId}`);
        if (orderRes.data.order) {
          fetchedOrder = orderRes.data.order;
          setOrder(fetchedOrder); 
          console.log("ADMIN: Order details fetched:", fetchedOrder);
        } else {
          console.error("ADMIN: Order not found in API response.");
          setError("Order not found.");
          setLoading(false);  
          setRatesLoading(false); 
          return; 
        }
        if (fetchedOrder?.pickupLocation && fetchedOrder.userId) {
          console.log("ADMIN: Fetching warehouse pincode for user:", fetchedOrder.userId, "location:", fetchedOrder.pickupLocation);
            try {
              const warehouseRes = await axios.get(`/api/admin/warehouses?userId=${fetchedOrder.userId}&search=${encodeURIComponent(fetchedOrder.pickupLocation)}`);
              if (warehouseRes.data.warehouses && warehouseRes.data.warehouses.length > 0) {
                ...
                console.log("ADMIN: Pickup pincode fetched:", fetchedPickupPincode);
              } else {
                console.warn("ADMIN: Could not find warehouse details for pickup location:", fetchedOrder.pickupLocation);
                setError("Could not determine pickup pincode. Please check the order's pickup location.");
              }
            } catch (whError) {
              console.error("ADMIN: Error fetching warehouse details:", whError);
              setError("Failed to fetch pickup location details.");
            }
          } else {
            if (!fetchedOrder?.userId) {
              console.warn("ADMIN: User ID missing from fetched order, cannot fetch warehouses.");
              setError("User ID missing from order, cannot fetch warehouses.");
            } else if (!fetchedOrder?.pickupLocation) {
              console.warn("ADMIN: Pickup location not set for this order.");
        ...
        if (fetchedOrder && fetchedPickupPincode) {
            ...
            try {
              ...
              console.log("ADMIN: Rate Payload:", ratePayload);
  
              const ratesRes = await axios.post<{ rates: RateResult[] }>('/api/user/courier-services', ratePayload);
              console.log("ADMIN: Rate API Response:", ratesRes.data);
              ...
              if (ratesRes.data && Array.isArray(ratesRes.data.rates)) {
                if (ratesRes.data.rates.length > 0) {
                  ...
                  console.log("ADMIN: Mapped Couriers:", mappedCouriers);
                } else {
                   console.warn("ADMIN: Rate API returned an empty array of rates.");
                   setAvailableCouriers([]);
                }
              } else {
                console.warn("ADMIN: No rates array received from API or invalid format.");
                setAvailableCouriers([]); 
              }
            } catch (rateError: any) {
              console.error("ADMIN: Error fetching courier rates:", rateError);
              toast.error(rateError.response?.data?.error || "Failed to fetch courier rates.");
              setAvailableCouriers([]);
            } finally {
                console.log("ADMIN: Skipping rate fetch because order or pickup pincode is missing.");
               setRatesLoading(false);  
            }
          } else {
            console.log("ADMIN: Skipping rate fetch because order or pickup pincode is missing.");
        ...
        setError("Failed to load order details for admin.");
        toast.error("Failed to load order details for admin.");
        setRatesLoading(false);  
      } finally {
        setLoading(false);
        console.log("ADMIN: Finished fetchOrderAndRates effect.");
      }
    };
  ...
  const handleConfirmShipment = async () => {
      ...
      try {
          const response = await axios.post('/api/admin/shipment/confirm', {
              orderId: order.id,
              userId: order.userId,
          ...
          if (response.status === 200 || response.status === 201) {
              ...
              setTimeout(() => router.push('/admin/dashboard/all-orders?status=shipped'), 3000);
          } else {
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
             ...
             <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 flex items-center">
                ...
                <Link href="/admin/dashboard/all-orders" className="hover:text-blue-600 dark:hover:text-blue-400">Orders</Link>
                <ChevronRight className="h-3 w-3 mx-1" />
        ...
        {!ratesLoading && availableCouriers.length > 0 && (
            <div className="mt-8 flex justify-end">
                <button
                    ...
                    className={`px-6 py-3 rounded-lg font-semibold text-white transition-colors duration-200 flex items-center justify-center shadow-md ${
