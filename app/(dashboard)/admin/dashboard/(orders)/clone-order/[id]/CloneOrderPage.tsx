...
export default function CloneOrderPageClient({ orderIdToClone }: CloneOrderClientProps) {
    ...
    if (pincode.length === 6) {
      ...
      try {
        ...
        if (data[0].Status === "Success") {
          ...
          toast.warn("Invalid pincode entered.");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.targetUserId) {
      return toast.error("Target user not identified. Cannot clone.");
    }
    ...
    if (form.paymentMode === "COD" && (!form.codAmount || Number(form.codAmount) <= 0)) {
    ...
    for (let i = 0; i < form.items.length; i++) {
        ...
        if (!item.productName) return toast.error(`Product Name is required for Item ${i + 1}`);
        if (!item.category) return toast.error(`Category is required for Item ${i + 1}`);
    ...
    if (!form.length || Number(form.length) <= 0) return toast.error("Length is required");
    if (!form.breadth || Number(form.breadth) <= 0) return toast.error("Breadth is required");
    if (!form.height || Number(form.height) <= 0) return toast.error("Height is required");
    if (!form.warehouseId) return toast.error("Please select a Pickup Location (Warehouse)");
    ...
    try {
      ...
      setTimeout(() => {
        router.push("/admin/dashboard/all-orders");
      }, 2000);
    } catch (error: any) {
      toast.update(tid, {
  ...
  const totalOrderValue = form.items.reduce(
    (acc, item) =>
      acc +
      (parseFloat(String(item.orderValue)) || 0) *
  ...
  return (
    <div className="mx-auto max-w-7xl animate-in fade-in slide-in-from-bottom-2 duration-700">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center px-4">
        <div>
          <h1 className="text-3xl font-black uppercase tracking-tighter text-[#0a0c37] dark:text-white">
            Clone Order (Admin)
          </h1>
          <p className="mt-1 text-sm font-bold text-gray-400">
            Target User: <span className="text-indigo-500 font-mono">{form.targetUserId}</span> | Cloning #{orderIdToClone}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-1 text-sm text-gray-500 dark:text-gray-400">
          <Link href="/admin/dashboard" className="flex items-center hover:text-[#0a0c37] transition-colors">
            <Home className="mr-1 h-3.5 w-3.5" />
            <span>Dashboard</span>
          </Link>
          <ChevronRight className="mx-1 h-3.5 w-3.5" />
          <Link href="/admin/dashboard/all-orders" className="hover:text-[#0a0c37] transition-colors">
            <span>All Orders</span>
          </Link>
          <ChevronRight className="mx-1 h-3.5 w-3.5" />
          <span className="font-medium text-gray-700 dark:text-gray-300">Clone</span>
        </div>
      </div>

      <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-xl shadow-indigo-900/5 dark:border-gray-800 dark:bg-gray-900 md:p-8 mx-4">
          ...
          <section className="transition-all duration-200">
            ...
            <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              <input
                ...
                onFocus={() => handleWarehouseFromDB(form.targetUserId)}
              />
            ...
            <div className="w-full">
              {warehouses.length === 0 ? (
                <div className="rounded-lg border border-dashed border-gray-300 py-4 text-center text-xs text-gray-500 dark:border-gray-700">
                  No warehouses found for the assigned user.
                </div>
              ) : (
