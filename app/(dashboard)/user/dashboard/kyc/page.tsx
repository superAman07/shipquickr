...
export default function KYC() {
  ...
  function isFormValid() {
    ...
    return (
      ...
      form.cheque &&
      (form.gst === "no" ||
        (form.gst === "yes" && form.gstNumber.trim() && form.gstCertificate)) &&
      form.gstCertificate &&
  ...
  const handler = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.status === 409) {
        const errorMsg =
          err.response?.data?.error || "KYC is already pending or approved.";
        toast.info(errorMsg);
      } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              ...
              {!isEditingMobile ? (
                <button
                  type="button"
                  className="text-blue-600 cursor-pointer hover:underline text-xs px-2 py-1"
                ...
                <button
                  type="button"
                  className="bg-purple-600 hover:bg-purple-700 cursor-pointer text-white text-xs px-3 py-1 rounded"
      ...
      <section className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-all">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Number Of Shipments {requiredField}
              </label>
              <select
              ...
              <div className="relative">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setForm({
                      ...form,
                      cheque: (e.target as HTMLInputElement).files?.[0] || null,
                    });
                  }}
                  required
                  type="file"
      ...
      <div className="flex justify-end mt-8">
        <button
          ...
          className={`px-6 cursor-pointer py-3 ${
            loading || !!accountError || !isFormValid()
              ? "bg-gray-400 cursor-not-allowed text-gray-200"
              : "bg-purple-600 hover:bg-purple-700 text-white cursor-pointer"
          } text-white font-medium rounded-lg shadow-sm transition-all flex items-center gap-2`}
        >
          {loading ? (
