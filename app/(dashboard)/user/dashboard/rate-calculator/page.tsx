...
export default function RateCalculator() {
  ...
  const validatePincode = async (pincode: string, setLocation: (loc: string) => void) => {
    if (pincode.length === 6) {
      try {
        ...
        setLocation("Error checking pincode");
      }
    } else {
      setLocation("");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (pickupLocation === "Invalid Pincode" || destLocation === "Invalid Pincode") {
      setError("Please fix the invalid pincodes before checking rates.");
      return;
    }
